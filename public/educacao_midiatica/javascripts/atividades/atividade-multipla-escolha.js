 ...
 */
function MultiplaEscolha (identificador, idQuestContainer, answer, feedAcerto, feedErro, config, elementos) {
...
MultiplaEscolha.prototype.AddEventMultQuestions = function () {
    ...
    for (i = 0; i < max; i++) {
        ...
        $.each(this.alternatives["p" + i], function (index, alternative) {
            ...
            //$(alternative).attr({"tabindex": 0, "aria-checked": false});
        });
...
MultiplaEscolha.prototype.setEvent = function (e) {
    ...
    this.feedback.empty().removeAttr('role', 'alert');
...
MultiplaEscolha.prototype.onClickbtnRefazer = function __onClickbtnRefazer(e) {
    ...
    $("#" + this.idQuestContainer +" .alternativa").removeClass('ativo feedCerto feedErrado').attr({
        'aria-checked': 'false',
        'aria-label': ariaLabel
    });
...
MultiplaEscolha.prototype.checkSelected = function (id) {

    var i = 0, max = this.currentAlter.length, elem;

    for (i = 0; i < max; i++) {
        elem = $(this.currentAlter[i]);
        if (id == elem.parent().data("option")) {
...
MultiplaEscolha.prototype.onClickAlternatives = function (e) {
    ...
    var id = $(e.target).parent().data("option");
...
MultiplaEscolha.prototype.checkSingleAnswer = function __checkSingleAnswer() {
    ...
    if (this.currentAlter.length > 0) {
        ...
        if (this.correto == this.answer.length) {
            ...
            //verifica se a atividade possui feeds individuais para cada alternativa
            if(this.feedErro  instanceof Array) {
                label = this.feedErro.filter(function(feed){ return feed.alt == select_alter})[0].feed;
            }
            else
                label = this.feedErro;
        }//else
...
MultiplaEscolha.prototype.checkMuitAnswer = function __checkMuitAnswer() {
    ...
    if (this.currentAlter.length > 0 && this.currentAlter.length == this.answer.length) {
        ...
        for (var j = 0; j < this.answer.length; j++) {
            ...
            self.saveData.gabaritos.push({"chave": (j+1), "valor": select_alter});
        ...
        $("#" + this.idQuestContainer +" .alternativa").unbind('click').unbind('keydown');
    } else {
        //Se nenhum elemento foi selecionado, o usuario recebe um alerta por feedback.
        label = 'Selecione uma alternativa.';
    }//else
...
MultiplaEscolha.prototype.activeFeed = function __activeFeed(label) {
...
MultiplaEscolha.prototype.saveAnswer = function __saveAnswer() {
 ...
 */
MultiplaEscolha.prototype.loadAnswer = function __loadAnswer(objResposta) {

    var setResposta = objResposta[0].respostas;

    var id, currResposta, respCerta, elem = "", self = this;

    //remove eventos de todas as alternativas
    $("#" + this.idQuestContainer +" .alternativa").unbind('click').unbind('keydown');

    //percorre alternativas e assinada a correta
    for (var j = 0; j < this.answer.length; j++) {

        currResposta = setResposta[j].valor;
        respCerta = (this.answer[j][0]).toString();

        $.each(this.alternatives['p' + j], function(index, alt){

            elem = $(alt);
            if((index+1).toString() == currResposta) {

                if(currResposta == respCerta)
                    elem.addClass('feedCerto');
                else
                    elem.addClass('feedErrado');
            }
        });
    }
